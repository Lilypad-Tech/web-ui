      ...
      return (
        <Fragment key={contributor?.id || contributor?.username}>
          ...
          >
            ...
            {!isAmbassador && (
              <td className="text-center px-4 py-2">
                {contributor.contributions?.split(";").filter((item) => item)
                  .length || "0"}
              </td>
            )}
            <td className="text-center px-4 py-2">
      ...
      <main className="container mx-auto rounded-xl border-secondary mt-4">
        <nav className="navbar md:w-full mb-6 md:mb-12 flex flex-col md:flex-row justify-between items-center gap-4">
          ...
          <div className="flex flex-col items-center gap-2">
            <div
              className="text-center text-sm md:text-md leading-7 text-[#E0FFF9] font-semibold antialiased"
              ...
              {loading ? (
                ...
                <span>
                  {`Total Contributions: ${contributors.reduce(
                    (total, contributor) => {
                      const count =
                        contributor?.contributions?.split(";").filter(Boolean)
                          .length || 0;
                      return total + count;
                    },
                    0
        ...
        <section
          className={`bg-[#181c21] md:min-h-[60vh] w-full flex flex-col my-auto overflow-x-auto transition-opacity duration-300`}
        >
          {loading ? (
