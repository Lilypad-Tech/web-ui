...
export default function NodeStatus() {
  ...
  const {
    ...
    queryFn: async () => getNodeLPBalances(nodesData?.map((n) => n.ID) ?? []),
  ...
  const {
    ...
    queryFn: async () => getNodeEthBalances(nodesData?.map((n) => n.ID) ?? []),
  ...
  return (
    <>
      <Head>
        <title>Lilypad Node Status - Monitor Your Node Performance!</title>
        <meta
          name="description"
              ...
              <div className="flex space-x-uui-5xl">
                <div className="flex-col flex space-y-uui-xs">
                  ...
                  <span className="text-uui-text-primary-900 uui-display-sm font-semibold">
                    {nodesIsLoading ? (
                      <RandomHexSpan length={4}></RandomHexSpan>
                    ) : nodesIsError ? (
                      <span>!err</span>
                    ) : (
                ...
                <div className="flex-col flex space-y-uui-xs">
                  ...
                  <span className="text-uui-text-primary-900 uui-display-sm font-semibold">
                    ...
                    {nodesIsLoading ? (
                      <RandomHexSpan length={2}></RandomHexSpan>
                    ) : nodesIsError ? (
                      <span>!err</span>
                    ) : (
                      <span>
                        {nodesData?.filter((node) => node.Online).length}
                      </span>
                    )}
                  </span>
                </div>
                ...
                <>
                  <thead className="">
                    ...
                    <tr className="sticky z-10 top-0 w-full after:w-full after:absolute after:inset-x-0 after:bottom-0 after:border-t-uui-1 after:border-t-uui-border-secondary">
                      {getHeaderData().map(
                        ({ name, translation, tooltip }, i, a) => (
                          <th
                            key={name}
                            className="p-0"
                            colSpan={i === a.length - 1 ? 2 : 1}
                          >
                            <TableHeaderCell title={translation}>
                              {tooltip ? (
                                <Tooltip
                                  text={tooltip.title}
                  ...
                  <tbody>
                    {toTableData({
                      ...
                      nodesData: nodesData ?? [],
                      lpBalances: nodesLPBalancesData ?? [],
                      ethBalances: nodesEthBalancesData ?? [],
                      powSubmissions: nodesPowSubmissionsData ?? [],
                    })
                      .filter((d) =>
                        walletAddress
                          ? d.Wallet.toLowerCase().includes(
                      ...
                      .map((row, rowIndex) => (
                        <tr key={rowIndex}>
                              ...
                              title={
                                <div className="flex space-x-uui-md">
                                  <Badge
                                    ...
                                    icon={{
                                      type: "icon",
                                      trailing: generalLinkExternal02,
                                    }}
                                  >
                                  ...
                                  <Badge
                                    ...
                                    icon={{
                                      type: "icon",
                                      trailing: copiedArray.find(
                                        (item) =>
                                          item.walletId === row["Wallet"]
                                      )
                                        ? generalCheck
                                        : generalCopy07,
                                    }}
                                    onClick={() => {
                                      setCopiedArray((copiedArray) => {
                                        const existingItem = copiedArray.find(
                                          (item) =>
                                            item.walletId === row["Wallet"]
                                        );
                          ...
                          <td>
                            <TableLeadText title={row["Available ETH"]} />
                          </td>
                          <td>
                            <TableLeadText title={row["Available LP"]} />
                          </td>
                          <td>
                            <TableLeadText
                              className="whitespace-nowrap"
                              title={row["Last POW submitted"]}
                            />
                          </td>
                          {/* <td>
														<TableLeadText
