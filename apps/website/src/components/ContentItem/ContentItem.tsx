...
const ContentItem = ({
    ...
    return (
        <div {...props}>
            {heading && (
                <h2
                    className={`${contentSizing[size].header} text-uui-text-primary-900 text-center font-semibold`}
                >
                    {heading}
                </h2>
            )}
            {paragraph && (
                <p
                    className={`${contentSizing[size].paragraph} text-uui-text-tertiary-600 txt font-regular text-center antialiased`}
                >
                    {paragraph}
                </p>
            )}
